...
const ShowReadCreatepopUp = () => {
    const operation = ["READ"];
    const [data, setData] = React.useState({ username: "kavi bartwal", email: "kavibartwal@gmail.com", });
    const [operations, setOperations] = React.useState({ read: true, create: false, });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const data = operations.create ? [...operation, "CREATE"] : operation;
        const combinedData = { accessList: data, userId: "abc" }; axios.post(constants.url, combinedData,
    ...
    return (<Paper style={{
        ...
        <MainCard title="Sample title">
            ...
            <Stack spacing={2} sx={{ width: 300 }}>
                ...
                >
                    <FormControlLabel value="Read" checked={true} control={<Checkbox />} label="Read" />
                    <FormControlLabel value="Create" onClick={handleToggle} control={<Checkbox />} label="Create" />
                </RadioGroup>
                <Button
                    variant="contained"
                    color="primary"
